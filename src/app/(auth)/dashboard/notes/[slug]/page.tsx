...
export default async function NoteDetailPage({
  ...
  const note = data.notes.find((note) => note.slug == slug);
    ...
    <section className="flex h-full w-full">
      ...
      <section className="p-4 flex-1 h-full flex item-start justify-start flex-col gap-4">
        ...
        <div className="text-muted-foreground flex items-center gap-2">
          <Tag className="w-4 h-4" /> Tags: {note?.tags.join(", ")}
        </div>
        <div className="text-muted-foreground flex items-center gap-2">
          <Calendar className="w-4 h-4" /> Created At: {note?.lastEdited}
        </div>
        <div className="border border-solid border-border w-full" />
        <article className="flex-1 text-foreground">{note?.content}</article>
